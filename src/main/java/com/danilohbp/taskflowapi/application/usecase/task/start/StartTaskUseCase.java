...
public class StartTaskUseCase {
    ...
    @Transactional
    public void execute(StartTaskCommand cmd) {
        if (cmd == null) {
            throw new IllegalArgumentException("StartTaskCommand is required");
        }
        ...
        if (!TaskPermission.canStart(actor, task)) {
            throw new BusinessRuleException("User cannot start this task");
        }

        // Regra de domínio: só permite start se status atual for TODO (por exemplo)
        task.start();
